...
// Finds this sheet's existing component for a given section (matched by
// name), or creates one. Each section gets its own properly-named group
// automatically — so tests from "Lipid Profile" and tests from
// "Hematology" always end up under their own correct heading, both in
// the editor and on the printed report, instead of piling into
// whichever single component happened to exist first.
function getOrCreateComponentForSection(sheet, section) {
  const normalizedSectionName = section.name.trim().toLowerCase();
  let component = sheet.tests.find((item) => item.name.trim().toLowerCase() === normalizedSectionName);
  if (!component) {
    component = makeComponent(section.name);
...
function toggleTestSelection(sheetId, sectionId, testId) {
  ...
  const activeComponent = getOrCreateComponentForSection(sheet, section);
...
function updateStylePreview() {
  ...
  const underline = document.getElementById('builderTestUnderline')?.classList.contains('active') || false;
...
function readBuilderForm() {
  ...
  return { 
    ...
    criticalOptions,
    formula,
    style: { isHeading, fontSize, fontFamily, alignment, bold, italic, underline }
  };
}
...
function clearBuilderForm() {
  ...
  const underlineBtn = document.getElementById('builderTestUnderline');
...
function renderTemplates() {
  ...
  const editingTestStyle = editingTest?.style || { isHeading: false, fontSize: 14, fontFamily: '', alignment: 'left', bold: false, italic: false, underline: false };
  editor.innerHTML = `
      ...
      <div class="panel-card">
        ...
        <div class="field-group" style="margin-top:16px;">
          ...
          <div style="margin-top:16px; padding:16px; background:var(--panel-strong); border-radius:10px;">
            ...
            <div class="inline-actions" style="margin-top:10px; gap:8px;">
              ...
              <button class="secondary-btn ${editingTestStyle.underline ? 'active' : ''}" id="builderTestUnderline" data-toggle="underline" style="text-decoration:underline;"><span style="text-decoration:underline;">U</span> Underline</button>
            </div>
        ...
        <div class="component-list" style="margin-top:16px;">
          ${currentSection.tests.length ? currentSection.tests.map((test) => {
            const testStyle = test.style || { isHeading: false, fontSize: 14, fontFamily: '', alignment: 'left', bold: false, italic: false, underline: false };
            ...
            return `<div class="component-card ${test.id === state.builderEditingTestId ? 'active' : ''}"><div class="component-card-header"><div><strong style="font-size:${fontSize}pt; font-family:${fontFamily}inherit; font-weight:${testStyle.bold ? 'bold' : 'normal'}; font-style:${testStyle.italic ? 'italic' : 'normal'}; text-decoration:${testStyle.underline ? 'underline' : 'none'}; display:block;">${escapeHtml(test.name)}</strong>${styleTag}<div class="muted-text" style="margin-top:4px;">${escapeHtml(test.unit || '—')} ${test.referenceRange ? `· ${formatMultilineText(test.referenceRange)}` : '· no reference range'}</div></div><div class="inline-actions"><button class="ghost-btn" data-edit-template-test="${test.id}">Edit</button><button class="ghost-btn" data-delete-template-test="${currentSection.id}" data-test-id="${test.id}">Delete</button></div></div></div>`;
          ...
          <div class="field-group">
            ...
            <div class="doctor-stack">
              ...
              <select data-doctor-select>
                ${doctorRegistry.doctors.map((doctor) => `<option value="${escapeHtml(doctor)}" ${doctor === activeSheet.patient.doctor ? 'selected' : ''}>${escapeHtml(doctor)}</option>`).join('')}
              </select>
            </div>
            <div class="inline-actions">
            ...
            <div class="component-card ${component.id === activeSheet.activeComponentId ? 'active' : ''}">
              ...
              <div class="component-subtest-list">
                ${component.tests.length ? component.tests.map((test) => {
                  ...
                  const testStyle = test.style || { isHeading: false, fontSize: 14, fontFamily: '', alignment: 'left', bold: false, italic: false, underline: false };
                  const fontSize = resolveFontSizePx(testStyle.fontSize);
                  const styleStr = `font-size:${fontSize}pt; font-family:${testStyle.fontFamily ? `'${testStyle.fontFamily}', ` : ''}inherit; text-align:${testStyle.alignment}; font-weight:${testStyle.bold ? 'bold' : 'normal'}; font-style:${testStyle.italic ? 'italic' : 'normal'}; text-decoration:${testStyle.underline ? 'underline' : 'none'};${testStyle.isHeading ? ' margin-top:12px; padding-top:8px; border-top:2px solid #e7eef8;' : ''}`;
        ...
        <tbody>
          ${source.tests.filter((component) => component.tests.length > 0).map((component) => `
            ${component.tests.map((test) => {
              ...
              const testStyle = test.style || { isHeading: false, fontSize: 14, fontFamily: '', alignment: 'left', bold: false, italic: false, underline: false };
              const fontSize = resolveFontSizePx(testStyle.fontSize);
              const styleStr = `font-size:${fontSize}pt; font-family:${testStyle.fontFamily ? `'${testStyle.fontFamily}', ` : ''}inherit; text-align:${testStyle.alignment}; font-weight:${testStyle.bold ? 'bold' : 'normal'}; font-style:${testStyle.italic ? 'italic' : 'normal'}; text-decoration:${testStyle.underline ? 'underline' : 'none'};${testStyle.isHeading ? ' padding-bottom:6px; padding-top:6px;' : ''}`;
...
function handleChange(event) {
  ...
  if (event.target.matches('input[data-field="patient.doctor"]')) {
    const activeSheet = getActiveSheet();
...
function handleClick(event) {
  ...
  if (target.matches('[data-toggle="bold"], [data-toggle="italic"], [data-toggle="underline"]')) {
