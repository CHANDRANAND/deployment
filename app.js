...
const AUTH_REDIRECT_URL = 'http://localhost:5500/index.html';
...
function clearBuilderForm() {
  ['builderTestName', 'builderTestUnit', 'builderTestRange', 'builderTestRangeMale', 'builderTestRangeFemale', 'builderTestOptions', 'builderTestAbnormalOptions', 'builderTestIsHeading', 'builderTestFontSize', 'builderTestAlignment'].forEach((id) => {
  ...
  editor.innerHTML = `
    <div class="editor-stack">
      ...
      <div class="panel-card">
        ...
        <div class="test-grid">
          ${currentSection.tests.slice(0, 10).map((test) => `
            ...
            <div class="component-card ${component.id === activeSheet.activeComponentId ? 'active' : ''}">
              ...
              <div class="component-subtest-list">
                ${component.tests.length ? component.tests.map((test) => {
                  ...
                  const valueField = test.options && test.options.length
                    ? `<select data-test-value="${component.id}" data-test-id="${test.id}" class="${abnormal ? 'value-abnormal' : ''}">
                    ...
                    : `<input data-test-value="${component.id}" data-test-id="${test.id}" value="${escapeHtml(test.value)}" class="${abnormal ? 'value-abnormal' : ''}" />`;
                  return `
                  <div class="component-subtest" ${testStyle.isHeading ? 'style="font-weight:700;"' : ''}>
                    <div>
                      <strong style="${styleStr}">${escapeHtml(test.name)}</strong>
                      <div class="muted-text">${escapeHtml(test.referenceRange || (test.options ? '' : test.unit) || '—')} ${abnormal ? '<span class="abnormal-badge">Outside range</span>' : ''}</div>
                    </div>
                    <div class="inline-actions">
                      ${valueField}
          ...
          ${source.tests.map((component) => `
            ...
            ${component.tests.length ? component.tests.map((test) => {
              ...
              <tr ${testStyle.isHeading ? 'style="font-weight:700;"' : ''}>
                ...
                <td class="param-value"><span class="meta-colon">:</span> ${abnormal ? `<strong>${displayValue}</strong>` : displayValue} ${escapeHtml(test.unit || '')}</td>
