...
function applyDoctorTemplate(sheetId, doctorName) {
  ...
  if (!hasAnyTests) {
    const componentName = resolvedTemplate.mainTests[0] || 'CBC';
...
function updateStylePreview() {
  ...
  preview.style.fontSize = `${resolveFontSizePx(fontSize)}px`;
      ...
      <div class="panel-card">
        ...
        <div class="field-group" style="margin-top:18px;">
          ...
          <div style="margin-top:16px; padding:16px; background:var(--panel-strong); border-radius:10px;">
            ...
            <div class="form-grid" style="gap:8px;">
              ...
              <div class="field-group"><label>Font size (px)</label><select id="builderTestFontSize">${Array.from({ length: 16 }, (_, i) => i + 7).map((n) => `<option value="${n}" ${resolveFontSizePx(editingTestStyle.fontSize) === n ? 'selected' : ''}>${n}px</option>`).join('')}</select></div>
            ...
            <div class="muted-text" style="margin-top:10px; font-size:0.8rem;">Preview: <span id="stylePreview" style="font-size:${resolveFontSizePx(editingTestStyle.fontSize)}px; font-family:${editingTestStyle.fontFamily ? escapeHtml(editingTestStyle.fontFamily) : 'inherit'}; text-align:${editingTestStyle.alignment}; font-weight:${editingTestStyle.bold ? 'bold' : 'normal'}; font-style:${editingTestStyle.italic ? 'italic' : 'normal'};">${editingTest ? escapeHtml(editingTest.name) : 'Preview text'}</span></div>
          </div>
        ...
        <div class="component-list" style="margin-top:16px;">
          ${currentSection.tests.length ? currentSection.tests.map((test) => {
            ...
            return `<div class="component-card ${test.id === state.builderEditingTestId ? 'active' : ''}"><div class="component-card-header"><div><strong style="font-size:${fontSize}px; font-family:${fontFamily}inherit; font-weight:${testStyle.bold ? 'bold' : 'normal'}; font-style:${testStyle.italic ? 'italic' : 'normal'}; display:block;">${escapeHtml(test.name)}</strong>${styleTag}<div class="muted-text" style="margin-top:4px;">${escapeHtml(test.unit || '—')} ${test.referenceRange ? `· ${formatMultilineText(test.referenceRange)}` : '· no reference range'}</div></div><div class="inline-actions"><button class="ghost-btn" data-edit-template-test="${test.id}">Edit</button><button class="ghost-btn" data-delete-template-test="${currentSection.id}" data-test-id="${test.id}">Delete</button></div></div></div>`;
            ...
            <div class="component-card ${component.id === activeSheet.activeComponentId ? 'active' : ''}">
              ...
              <div class="component-subtest-list">
                ${component.tests.length ? component.tests.map((test) => {
                  ...
                  const testStyle = test.style || { isHeading: false, fontSize: 14, fontFamily: '', alignment: 'left', bold: false, italic: false };
                  const fontSize = resolveFontSizePx(testStyle.fontSize);
                  const styleStr = `font-size:${fontSize}px; font-family:${testStyle.fontFamily ? `'${testStyle.fontFamily}', ` : ''}inherit; text-align:${testStyle.alignment}; font-weight:${testStyle.bold ? 'bold' : 'normal'}; font-style:${testStyle.italic ? 'italic' : 'normal'};${testStyle.isHeading ? ' margin-top:12px; padding-top:8px; border-top:2px solid #e7eef8;' : ''}`;
        ...
        <tbody>
          ${source.tests.map((component) => `
            ...
            ${component.tests.length ? component.tests.map((test) => {
              ...
              const testStyle = test.style || { isHeading: false, fontSize: 14, fontFamily: '', alignment: 'left', bold: false, italic: false };
              const fontSize = resolveFontSizePx(testStyle.fontSize);
              const styleStr = `font-size:${fontSize}px; font-family:${testStyle.fontFamily ? `'${testStyle.fontFamily}', ` : ''}inherit; text-align:${testStyle.alignment}; font-weight:${testStyle.bold ? 'bold' : 'normal'}; font-style:${testStyle.italic ? 'italic' : 'normal'};${testStyle.isHeading ? ' border-bottom:1px solid #999; padding-bottom:6px; padding-top:6px;' : ''}`;
