...
export function AuthProvider({ children }) {
  ...
  const signOut = async () => {
    if (supabaseClient) {
      try {
        // 'local' scope clears the session on this device immediately,
        // without waiting on a network round-trip to revoke it
        // server-side first. The default 'global' scope needs that
        // round-trip to succeed before the local session clears — if
        // it's slow or fails, the old session token can be left behind,
        // causing login to silently fail right after logout until
        // storage is cleared or an incognito window is used.
    ...
    try {
      ...
      console.warn('Could not clear stale Supabase session keys', e);
    }
