...
export function AppProvider({ children }) {
  ...
  // Choosing a doctor should automatically switch to their custom
  // template (tests, print layout, signature) if one exists — that's
  // the entire point of Doctor Templates. If the sheet already has
  // tests entered, we only switch which template new picks come from;
  // we never wipe out work already in progress just because the
  // doctor field changed.
  const getTemplateForDoctor = (doctorName) => {
    const normalized = (doctorName || '').trim().toLowerCase();
    if (!normalized) return templates.find((t) => !t.forDoctor) || templates[0];
    const custom = templates.find((t) => t.forDoctor && t.forDoctor.trim().toLowerCase() === normalized);
    return custom || templates.find((t) => !t.forDoctor) || templates[0];
  };

  const setActiveSheetDoctor = (doctorName) => {
    setSheets((prev) =>
      prev.map((sheet) => {
        if (sheet.id !== activeSheetId) return sheet;
        ...
        const nextTests = hasAnyTests || resolvedTemplate.id === sheet.templateId
          ? sheet.tests
          : JSON.parse(JSON.stringify(resolvedTemplate.sections || []));
        return {
          ...sheet,
          templateId: resolvedTemplate.id,
          tests: nextTests,
          activeComponentId: hasAnyTests ? sheet.activeComponentId : (nextTests[0]?.id || ''),
          patient: { ...sheet.patient, doctor: doctorName }
        };
      })
    );
  };
  ...
  const saveActiveSheetToHistory = () => {
    ...
    const snapshot = {
      historyId: activeSheet.id, // stable per-sheet id, so repeat saves update this same entry
    ...
    setHistory((prev) => {
      const existingIndex = prev.findIndex((entry) => entry.historyId === snapshot.historyId);
      if (existingIndex >= 0) {
        const next = [...prev];
